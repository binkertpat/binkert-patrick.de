...
const PortableBitmapEditor = () => {
  ...
  const parseP1 = () => {
    ...
    pbmData.DATA.forEach((line, i) => {
      ...
      Array.from(line).forEach((pixel) => {
        ...
        if (pixel === '1' || pixel === '0') {
          ctxData.fillRect(x * pixelDimension, i * pixelDimension, pixelDimension, pixelDimension);
          x++;
        }
      })
      x = 0;
  ...
  const parsePbmInput = () => {
    ...
    setMaxRangeSlider(parseInt(document.querySelector('textarea').offsetWidth / width));
    setPbmData({...pbmData, ...{ MODE: mode, WIDTH: width, HEIGHT: height, DATA: data}});
  }

  const handlePixelDimensionChange = (e) => {
    if(parseInt(e.target.value) * pbmData.WIDTH > document.querySelector('textarea').offsetWidth) {
      return;
    }
  ...
  return (
    <>
      ...
      <textarea className="form-control mb-3 textarea-with-code" id="pbm_input" rows="12" value={pbmInput} onChange={(e) => setPbmInput(e.target.value)}></textarea>
      ...
      {showCanvasAndOptions && (
        <>
          <div className="mb-3">
            <label htmlFor="dimension" className="form-label">Größe ändern</label>
            <input type="range" className="form-range" id="dimension" min="1" max={maxRangeSlider} value={pixelDimension} onChange={handlePixelDimensionChange}/>
          </div>

          <canvas id="pbm_canvas" ref={ctx} className="mb-3" style={{border: '1px solid black'}} width={pbmData.WIDTH * pixelDimension} height={pbmData.HEIGHT * pixelDimension}></canvas>

          <div className="accordion mb-3" id="debugOutput">
            <div className="accordion-item">
              <h2 className="accordion-header">
                <button className="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#debugOutputOne" aria-expanded="false" aria-controls="debugOutputOne">
                  Debug Output
                </button>
              </h2>
              <div id="debugOutputOne" className="accordion-collapse collapse" data-bs-parent="#debugOutput">
